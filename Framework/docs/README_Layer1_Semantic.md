...
**Row accessibility identifiers (UI tests):** You can steer which model property feeds automatic row `accessibilityIdentifier` segments via `PresentationHints.customPreferences` (`itemAccessibilityIdentifierProperty`, `itemAccessibilityIdentifierDefault`). Defaults follow title resolution plus stable `id`. See [AutomaticAccessibilityIdentifiers.md](AutomaticAccessibilityIdentifiers.md#collection-rows-and-data-driven-identifier-segments) and the [Migration Guide](MigrationGuide.md#accessibility-identifiers-for-collection-rows) when upgrading.
