...
open class ConsolidatedAccessibilityTests: BaseTestClass {
    ...
    @Test @MainActor func testPlatformPresentModalFormL1GeneratesAccessibilityIdentifiersOnIOS() async {
        self.initializeTestConfig()
        // Given: modal form presentation (ModalFormTestData / PresentationHints reserved for future field-level assertions)
        _ = ModalFormTestData(name: "Test Name", email: "test@example.com")
        _ = PresentationHints(
            dataType: .form,
            presentationPreference: .automatic,
            complexity: .moderate,
    ...
    @Test @MainActor func testPlatformPresentModalFormL1GeneratesAccessibilityIdentifiersOnMacOS() async {
        self.initializeTestConfig()
        // Given: modal form presentation (fixtures kept for parity with iOS test)
        _ = ModalFormTestData(name: "Test Name", email: "test@example.com")
        _ = PresentationHints(
            dataType: .form,
            presentationPreference: .automatic,
    ...
    func testPlatformPhotoPickerL4ReturnsCorrectPlatformImplementation(
        ...
        self.initializeTestConfig()
        // When
        _ = PlatformPhotoComponentsLayer4.platformPhotoPicker_L4(
            onImageSelected: { _ in }
        )
    ...
    @Test @MainActor func testGenericItemCollectionViewGeneratesAccessibilityIdentifiers() async {
        ...
        let testItems = [
            ...
            TestPatterns.TestItem(id: "Item 2", title: "Item 2"),
            TestPatterns.TestItem(id: "Item 3", title: "Item 3")
        ]
        let hints = PresentationHints()
    ...
    @Test @MainActor func testExpandableCardCollectionViewGeneratesAccessibilityIdentifiers() async {
        self.initializeTestConfig()
            ...
            let testItems = [
                ...
                TestPatterns.TestDataItem(title: "Card 2")
            ]
            let hints = PresentationHints()
    ...
    @Test @MainActor func testCoverFlowCollectionViewGeneratesAccessibilityIdentifiers() async {
        self.initializeTestConfig()
            ...
            let testItems = [
                ...
                TestPatterns.TestDataItem(title: "CoverFlow Card 2")
            ]
            let hints = PresentationHints()
    ...
    @Test @MainActor func testGridCollectionViewGeneratesAccessibilityIdentifiers() async {
        self.initializeTestConfig()
            ...
            let testItems = [
                ...
                TestPatterns.TestDataItem(title: "Grid Card 2")
            ]
            let hints = PresentationHints()
    ...
    @Test @MainActor func testSwitchControlGesture() {
        ...
        #expect(gesture.timestamp.timeIntervalSince1970 > 0)
    }
    ...
    @Test @MainActor func testButtonsInListItemsGetUniqueIdentifiers() {
        ...
        if let inspected1 = try? AnyView(button1).inspect(),
           let _ = try? inspected1.accessibilityIdentifier(),
           let inspected2 = try? AnyView(button2).inspect(),
           let _ = try? inspected2.accessibilityIdentifier() {
    ...
    @Test @MainActor func testForEachListItemsGetUniqueIdentifiers() {
        ...
        let items = [
            TestPatterns.TestItem(id: "1", title: "Alpha"),
            TestPatterns.TestItem(id: "2", title: "Beta"),
            TestPatterns.TestItem(id: "3", title: "Gamma")
        ]
        ...
        #if canImport(ViewInspector)
        if let inspected = try? AnyView(listView).inspect() {
    ...
    @Test @MainActor func testGridCollectionItemsGetUniqueIdentifiers() {
        ...
        let items = [
            TestPatterns.TestItem(id: "grid-1", title: "Grid Item 1"),
            TestPatterns.TestItem(id: "grid-2", title: "Grid Item 2"),
            TestPatterns.TestItem(id: "grid-3", title: "Grid Item 3")
        ]
    ...
    @Test @MainActor func testPlatformTabStripPickerSegmentsGetAccessibilityIdentifiers() {
        ...
        #if canImport(ViewInspector)
        if (try? AnyView(tabStrip).inspect()) != nil {
    ...
    @Test @MainActor func testDynamicEnumFieldPickerSegmentsGetAccessibilityIdentifiers() {
        ...
        #if canImport(ViewInspector)
        if (try? AnyView(enumField).inspect()) != nil {
            // The picker should exist and have segments with accessibility identifiers
            // Since ViewInspector can't easily inspect Picker ForEach content, we verify the implementation
    ...
    @Test @MainActor func testVoiceControlCommandRecognition() {
        ...
        #expect(recognition.timestamp.timeIntervalSince1970 > 0)
    }
    ...
    @Test @MainActor func testListCollectionViewGeneratesAccessibilityIdentifiers() async {
        self.initializeTestConfig()
            ...
            let testItems = [
                ...
                TestPatterns.TestDataItem(title: "List Card 2")
            ]
            let hints = PresentationHints()
    ...
    @Test @MainActor func testMasonryCollectionViewGeneratesAccessibilityIdentifiers() async {
        self.initializeTestConfig()
            ...
            let testItems = [
                ...
                TestPatterns.TestDataItem(title: "Masonry Card 2")
            ]
            let hints = PresentationHints()
    ...
    @Test @MainActor func testAdaptiveCollectionViewGeneratesAccessibilityIdentifiers() async {
        self.initializeTestConfig()
            ...
            let testItems = [
                ...
                TestPatterns.TestDataItem(title: "Adaptive Card 2")
            ]
            let hints = PresentationHints()
    ...
    @Test @MainActor func testGetDebugLogMethodExists() async {
        self.initializeTestConfig()
            ...
            let debugLog = config.getDebugLog()
            #expect(type(of: debugLog) == String.self, "getDebugLog should return a String")
        }
    }
 ...
 #expect(hasAccessibilityID, "platformPresentHierarchicalData_L1 with enhanced hints and custom view should generate accessibility identifiers ")
    ...
    @Test @MainActor func testExactNamedBehavior() {
        ...
        #if canImport(ViewInspector)
        let ids1 = AccessibilityTestUtilities.allAccessibilityIdentifiersFromViewInspector(view1)
        let ids2 = AccessibilityTestUtilities.allAccessibilityIdentifiersFromViewInspector(view2)
        #expect(ids1.contains("SameName"), "exactNamed() should produce exact identifier 'SameName', got \(ids1)")
        #expect(ids2.contains("SameName"), "exactNamed() should produce exact identifier 'SameName', got \(ids2)")
        #else
        // ViewInspector not available on this platform (likely macOS) - this is expected, not a failure
        #endif
    }
    ...
    @Test @MainActor func testExactNamedVsNamedDifference() {
        ...
        #if canImport(ViewInspector)
        let exactIDs = AccessibilityTestUtilities.allAccessibilityIdentifiersFromViewInspector(exactView)
        let namedIDs = AccessibilityTestUtilities.allAccessibilityIdentifiersFromViewInspector(namedView)
        let exactID = exactIDs.first(where: { $0 == "TestButton" || $0.contains("TestButton") })
        let namedID = namedIDs.first(where: { $0.contains("TestButton") })
        #expect(exactID != nil && namedID != nil, "Should find exactNamed and named identifiers")
        if let exactID, let namedID {
    ...
    @Test @MainActor func testExactNamedIgnoresHierarchy() {
        ...
        #if canImport(ViewInspector)
        let exactIDs = AccessibilityTestUtilities.allAccessibilityIdentifiersFromViewInspector(exactView)
        let exactID = exactIDs.first(where: { $0.contains("SaveButton") }) ?? exactIDs.first
    ...
    @Test @MainActor func testExactNamedMinimalIdentifier() {
        ...
        #if canImport(ViewInspector)
        let exactIDs = AccessibilityTestUtilities.allAccessibilityIdentifiersFromViewInspector(exactView)
        #expect(exactIDs.contains("MinimalButton"), "exactNamed() should produce exact identifier 'MinimalButton', got \(exactIDs)")
        #else
        // ViewInspector not available on this platform (likely macOS) - this is expected, not a failure
        #endif
    }
    ...
    @Test @MainActor func testNestedNamedCalls() {
        ...
        #if canImport(ViewInspector)
        let buttonIDs = AccessibilityTestUtilities.allAccessibilityIdentifiersFromViewInspector(view)
        let buttonID = buttonIDs.first(where: { $0.contains("DeepNested") || $0.contains("SixLayer") }) ?? buttonIDs.first
 ...
 #expect(hasAccessibilityID, "Framework component (platformPresentBasicValue_L1) should generate accessibility identifiers ")         #else
    ...
    @Test @MainActor func testAllLayer1Functions_AutomaticHIGCompliance() async {
        ...
        _ = self.hostRootPlatformView(collectionView.enableGlobalAutomaticCompliance())
        ...
        _ = self.hostRootPlatformView(numericView.enableGlobalAutomaticCompliance())
    ...
    @Test @MainActor func testPlatformOptimizationSettingsGeneratesAccessibilityIdentifiers() async {
        ...
        _ = PlatformOptimizationSettings(for: .iOS)
 ...
 #expect(hasAccessibilityID, "PlatformRecommendationEngine should generate accessibility identifiers ")
    ...
    @Test @MainActor func testAccessibilityIdentifiersIncludeLabelTextForStringLabels() {
        ...
        #if canImport(ViewInspector)
        let submitIDs = AccessibilityTestUtilities.allAccessibilityIdentifiersFromViewInspector(submitButton)
        let cancelIDs = AccessibilityTestUtilities.allAccessibilityIdentifiersFromViewInspector(cancelButton)
        let submitID = submitIDs.first
        let cancelID = cancelIDs.first
        #expect(submitIDs.contains(where: { $0.localizedCaseInsensitiveContains("submit") }), "Submit button identifier should include 'Submit' label")
        #expect(cancelIDs.contains(where: { $0.localizedCaseInsensitiveContains("cancel") }), "Cancel button identifier should include 'Cancel' label")
        if let submitID, let cancelID {
 ...
 #expect(hasAccessibilityID, "DragDropManager should generate accessibility identifiers ")
    ...
    @Test @MainActor func testGlobalConfigActuallyControlsIdentifierGeneration() async {
        ...
        #if canImport(ViewInspector)
        let enabledIDs = AccessibilityTestUtilities.allAccessibilityIdentifiersFromViewInspector(testView2)
        let accessibilityIdentifier2 = enabledIDs.first(where: { $0.hasPrefix("test.") }) ?? enabledIDs.first
        #expect(accessibilityIdentifier2 != nil && !(accessibilityIdentifier2?.isEmpty ?? true), "An identifier should be generated when enabled")
        if let accessibilityIdentifier2 {
...
// NOTE: GenericItemCollectionViewRealAccessibilityTDDTests is defined in GenericItemCollectionViewAccessibilityTests.swift
