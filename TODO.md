...
- [ ] **#209** Managed settings flow — done: flow logic, top-level state (`CaseIterable` compile-time init), `anyHashableBinding`, `platformManagedSettingsTopLevel_L4`, iPhone stack helper + `createSettingsContainerForiPhone`; pending: sub-pane stack API, result-builder or richer pane tree, example/docs, full acceptance
