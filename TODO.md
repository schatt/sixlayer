...
- [ ] **#209** Managed settings flow — done: flow logic, top-level state (`CaseIterable` init), `anyHashableBinding`, `platformManagedSettingsTopLevel_L4`, iPhone stack helper, `PlatformManagedSettingsDetailNavigationState` (sub-pane path); pending: wire sub-pane into Layer 4 / `NavigationStack(path:)`, result-builder or richer pane tree, example/docs, full acceptance
